jQuery(document).ready(function($) {
    $(".drop-icon").click(function() {
        var dropIcon = $(this);
        var fileInput = dropIcon.parent().find(".fileInput");

        if (fileInput.length > 0) {
            fileInput.click();
            fileInput.change(function() {
                var file = this.files[0];
                var fileNameElement = dropIcon.parent().find(".file-name");

                if (fileNameElement.length > 0 && file) {
                    var reader = new FileReader();
                    reader.onload = function(e) {
                        fileNameElement.empty(); // Clear any previous content
                        if (file.type === 'application/pdf') {
                            var image = $("<img>").attr("src", 'data:image/png;base64,').addClass("uploadImage").appendTo(fileNameElement);
                        } else {
                            var image = $("<img>").attr("src", e.target.result).addClass("uploadImage").appendTo(fileNameElement);
                        }
                        var crossIcon = $("<span>").addClass("cross-icon").html(' <i class="fa fa-times" aria-hidden="true"></i>').appendTo(fileNameElement);

                        crossIcon.click(function() {
                            fileInput.val(""); // Clear the file input
                            dropIcon.show(); // Show the drop-icon box
                            fileNameElement.empty(); // Remove the image and cross icon
                        });

                        dropIcon.hide(); // Hide the drop-icon box
                    };
                    reader.readAsDataURL(file);
                }
            });
        }
    });
});


jQuery(document).ready(function ($) {
    $("#odometerUnknown").click(function () {
        var odometerUnknown = $(this).prop("checked");
        var odometerInput = $(".odometer");
        if (odometerUnknown) {
            $('#odometer').val("Unknown").prop("readonly", true);
        } else {
            odometerInput.val("").prop("readonly", false);
        }
    });

    $("#priorNadaTradeInValue").keyup(function () {
        var priorNadaTradeInValue = $(".priorNadaTradeInValue");
        if (priorNadaTradeInValue && priorNadaTradeInValue.val() !== '' ) {
            if ( !/^(\$).*$/.test(priorNadaTradeInValue.val()) ) {
                priorNadaTradeInValue.val('$' + priorNadaTradeInValue.val());
            }
        }
    });

    $("#priorNadaTradeInValueMSL").click(function () {
        var priorNadaTradeInValue = $(".priorNadaTradeInValue");
        if (priorNadaTradeInValue && priorNadaTradeInValue.val() !== "" ) {
            alert('You can\'t select this option if you have entered a value for the Prior NADA Loan Value.');
            $(this).prop("checked", false);
        }
    });

    $("#priorNadaTradeInValueUnknown").click(function () {
        var priorNadaTradeInValue = $(".priorNadaTradeInValue");
        if (priorNadaTradeInValue && priorNadaTradeInValue.val() !== "" ) {
            alert('You can\'t select this option if you have entered a value for the Prior NADA Loan Value.');
            $(this).prop("checked", false);
        }
    });
});

jQuery(document).ready(function ($) {
    $(".appraisal .drop-area .file-name i").click(function () {
        var deleteImageUrl = $(this).closest('.file-name').attr("data-url");
        var $fileName = $(this).closest('.file-name');
        var $dropIcon = $fileName.siblings('.drop-icon');
        var urlParams = new URLSearchParams(deleteImageUrl);
        var fileKey = urlParams.get('filekey');
        if (confirm("Are you sure you want to delete this attachment?")) {
            $.ajax({
                url: deleteImageUrl,
                type: 'get',
                data: '',
                success: function (response) {
                    $fileName.empty();
                    $dropIcon.removeClass('hidden');
                    $('#' + fileKey).val('');
                },
                error: function (xhr, status, error) {
                    console.error("Error occurred:", error); // Log any errors
                }
            });
        }
    });
});

